...
//	전체호텔 위시리스트 체크여부
	public void wishList2(List<HotelDTO> hotelList, int u_num){
		List<Integer> h_num = new ArrayList<Integer>();
		for(int i=0; i < hotelList.size(); i++) {
			HotelDTO hdto = hotelList.get(i);
			h_num.add(hdto.getH_num());
		...
		List<WishListDTO> wishList = sqlSession.selectList("wishList2", u_num);
		for(int i=0; i<hotelList.size(); i++) {
			HotelDTO hdto = hotelList.get(i);
...
//	객실수량내리기
	public void roomCountDown(Map<String,String> params) {
		sqlSession.update("roomCountDown", params);
	}
...
//	유저의 예약정보 반환2
	public BookingDTO bookInfo2(int u_num,int h_num) {
		Map<String, Integer> map = new Hashtable<String, Integer>();
		map.put("u_num", u_num);
		map.put("h_num", h_num);
		BookingDTO bdto = sqlSession.selectOne("bookInfo2", map);
		return bdto;
	}
...
//	예약취소
	public int deleteBook(int book_num, int u_num) {
		...
		//객실수량올리기
		int room_num = sqlSession.selectOne("findRoomNum", book_num);
		sqlSession.update("roomCountUp", room_num);
...
//	객실가격반환
	public int getRoomPrice(Map<String, String> params) {
		int room_num = sqlSession.selectOne("getRoomNum",params);
